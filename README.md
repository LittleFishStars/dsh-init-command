...
- **阶段二（AGENTS.md 生成）默认只显示完成信息**：生成的 `AGENTS.md` 内容不进入会话日志，直接写入文件（silent 流式执行），生成成功后追加一条完成信息（notice 折叠行：路径、字符数）与一条形如模型正式输出的成功状态消息（`assistant/message` 呈现，文本由插件合成：文件、字符数、项目类型与 `--dry-run` 预览提示）；只有 `--dry-run` 时才与阶段一一样完整流式显示，便于预览
- **全程可追溯**：阶段一的提示词、输出、token 用量与阶段二的完成信息、成功状态消息以标准会话事件呈现；调用记录（阶段、路由、提示词摘要、结果）随命令结果持久化，重启后依旧安全加载（全部事件类型都在会话恢复白名单 `KNOWN_SESSION_EVENT_TYPES` 内）
...
- **默认直接写文件**：阶段二（生成）默认静默执行，`AGENTS.md` 内容不输出到对话，直接写入工作区，仅在会话中追加完成信息与成功状态消息；`--dry-run` 时完整流式预览
...
- **模型调用写入对话历史**：阶段一的完整对话（提示词、流式输出、token 用量）与阶段二的完成信息、成功状态消息作为标准会话事件持久化，重启后安全加载；两次调用随命令结果（`command/done` 事件）附带摘要（阶段、provider/model、提示词行数与首行摘要、模型结果）。事件类型全部位于会话恢复白名单（`KNOWN_SESSION_EVENT_TYPES`）内——直接 `session.append()` 自定义事件类型会被会话恢复路径拒绝加载，而仓库外插件无法把自定义类型注册进白名单
...
| 命令 | 行为 |
| --- | --- |
| `/init` | 两阶段调用模型，在工作区根目录生成或替换 `AGENTS.md`（已存在时直接覆盖，旧内容作为改写参考）；生成内容默认直接写入文件，会话中显示完成信息与成功状态消息（完整内容用 `--dry-run` 预览）；阶段一默认关闭思考模式 |
