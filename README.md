...
- **阶段二（AGENTS.md 生成）默认只显示思考过程**：阶段二提示词同样以可折叠 notice 行进入会话，模型的 **reasoning 流块实时转发可见**（思考过程一目了然），但 `AGENTS.md` 的正文输出不进入会话日志——text 流块被过滤、也不追加最终 `assistant/message`，内容直接写入文件；生成成功后追加一条完成信息（notice 折叠行：路径、字符数）与一条形如模型正式输出的成功状态消息（`assistant/message` 呈现，文本由插件合成：文件、字符数、项目类型）；只有 `--dry-run` 时才与阶段一一样完整流式显示正文，便于预览
