...
/**
 * 追加一个合成显示的 step：一条 notice（折叠行摘要）与一条形如模型正式
 * 输出的消息（成功状态、帮助、询问等插件合成的文本）。事件序列与阶段一
 * 一致：`step/start` → `user/message`（notice）→ `assistant/message` →
 * `step/end`；不写 turn 边界（见 {@link INIT_TURN}）。
 *
 * 消息以 `assistant/message` 呈现且来源复用模型路由：持久化加载路径要求
 * `assistant/message` 必须携带 model 来源（插件来源会被拒绝）。文本由插件
 * 合成，非模型生成。
 * @param {{ summary: string, text: string }} notice - notice：summary 为折叠行摘要，text 为展开文本。
 */
export function appendNoticeStep(session, step, route, notice, message) {
  session.append('step/start', { turn: INIT_TURN, step })
  session.append('user/message', {
    id: randomUUID(),
    role: 'user',
