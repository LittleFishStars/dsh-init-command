...
/**
 * 阶段二提示词：参考 opencode 的 /init 命令，嵌入判断结果生成 AGENTS.md。
 * @param {{ notImplemented?: boolean }} [options] - `notImplemented` 为 true 时
 *   说明项目目录为空、尚未制作：提示模型把 AGENTS.md 写成基于用户描述的规划稿，
 *   并在文档中明确注明项目尚未创建。
 */
export function generatePrompt(treeText, profile, existing, options = {}) {
  const { notImplemented = false } = options
  const detail = [
  ...
  return [
    ...
    ...notImplemented
      ? ['', 'IMPORTANT: the project directory is currently EMPTY — the project has not been implemented yet. Write the AGENTS.md as a plan/blueprint for the project described by the user. State clearly at the top of the document that this project has not been created yet (此项目还未制作/尚未实现), and base every detail on the user description above — do not invent anything beyond it.']
      : [],
    '',
