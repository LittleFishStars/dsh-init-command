...
/** 帮助消息的来源回退（无可用模型路由时）：占位 provider/model 满足持久化校验。 */
const HELP_SOURCE = { provider: 'dsh', model: 'init-help' }

/**
 * 展示 `/init` 帮助。命令结果卡片会把多行文本折叠成一行摘要（点击展开，
 * 见 DSH 的 GenericCommandCard），因此按完成信息的模式把帮助写进会话：
 * 以 `assistant/message` 完整展开显示。
 */
function showHelp(config, invocation) {
  const session = invocation.agent?.session
  if (session) {
    const route = resolveRoute(config, invocation.agent) ?? HELP_SOURCE
    appendCompletionNotice(session, nextInitStep(session), route, {
      summary: '帮助：/init 用法',
